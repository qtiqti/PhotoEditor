...
import os
import shutil # позволяет перемещать файлы даже с разных дисков, в то время как os.move только на данном диске
import pyperclip
import numpy as np
...
class PyPhotoEditor:
    def __init__(self):
        ...
        self.selection_top_x = 0 # координаты прямоугольника для выделения
    ...
    def init(self):
        ...
        self.root.iconphoto(True, PhotoImage(file="resources/camera.png"))
    ...
    def draw_menu(self):
        ...
        clipboard_menu = Menu(file_menu, tearoff=0) # буфер обмена
    ...
    def open_new_images(self):
        image_paths = fd.askopenfilenames(filetypes=(('png files', '*.png'),('jpeg files', '*.jpeg'), ('jpg files', '*jpg'), ))
    ...
    def close_current_image(self): # метод для закрытия отдельной картинки
        current_tab, path, image = self.get_current_working_data() # получаем индекс текущей вкладки, путь до картинки и саму картинку
        if not current_tab:
            return
        index = self.image_tabs.index(current_tab) # порядковый номер картинки узнаем, а он также равен номеру вкладки

        image.close()
        del self.opened_images[index] # удаляем картинку из списка открытых (оператор del)
        self.image_tabs.forget(current_tab) # eдалим вкладку, чтобы она не рисовалась
    ...
    def move_current_image(self):
        ...
        new_dir = fd.askdirectory(initialdir=os.path.dirname(old_path)) # запросим путь до папки : для начала, начальный путь - старый
        if not new_dir:
            return

        new_path = os.path.join(new_dir, os.path.split(old_path)[1]) # функция join позволяет соеденить два пути , те он соеденяет путь с названием картинки
    ...
    def start_area_selection_of_current_image(self):
        current_tab = self.image_tabs.select() # получаем доступ к canvas
        if not current_tab:
            return
        tab_frame = self.image_tabs.children[current_tab[current_tab.rfind('!'):]]
        canvas = tab_frame.children['!canvas']
        ...
        canvas.bind("<Button-1>", self.get_selection_start_pos) # чтобы получить координаты точек, мы фиксируем точку в момент зажатия ЛКМ и вторую в момент отпускания кнопки
    ...
    def update_selection_end_pos(self, event):
        ...
        if self.canvas_for_selection is not None and self.selection_rect is not None:
            self.canvas_for_selection.coords(        # обновляем прямоугольник
                self.selection_rect,
                self.selection_top_x, self.selection_top_y,
                self.selection_bottom_x, self.selection_bottom_y
            )
    ...
    def stop_area_selection_of_current_image(self):
        ...
        self.selection_rect = None # все обнуляем, тк мы закончили выделение
        self.canvas_for_selection = None
    ...
    def crop_current_image(self):
        ...
        image = image.crop((
            self.selection_top_x, self.selection_top_y,
            self.selection_bottom_x, self.selection_bottom_y
        )) # одни скобки относятся к методу crop другие к координатам
    ...
    def convert_current_image(self, mode):
        ...
        if mode == "roll":
            ...
            image = Image.fromarray(np.array(image)[:,:,::-1])# превращаем картинку в массив, крутим , превращаем обратно в картинку
            self.update_image_inside_app(current_tab, image)
            return

        elif mode in "R G B".split(' '):  # сделаем, чтобы можно было отделять каналы
            if image.mode != "RGB":
                mb.showerror("RGB Roll error", f"Can't split channel of image with not RGB mode '{image.mode}'")
                return

            a = np.array(image)
            a[:,:,(mode!="R", mode!="G", mode!="B")] *= 0 #где режим будет равен букве, будет стоять 1 и все пиксели кроме 1 умножаем на 0
